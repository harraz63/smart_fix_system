...
export const getHome = async (_req: Request, res: Response): Promise<void> => {
  const [categories, featuredServices, topProfiles] = await Promise.all([
    ...
    Service.find({ isActive: true }).populate('categoryId', 'name icon technicianType').limit(6),
  ...
  const banners = [
    { id: 1, title: 'Fix it Fast',             subtitle: 'Professional technicians near you',    imageUrl: null, type: TechnicianType.Plumbing },
    { id: 2, title: '20% Off Painting',         subtitle: 'Book a painter this week',             imageUrl: null, type: TechnicianType.Painting },
    { id: 3, title: 'AC Service & Repair',      subtitle: 'Stay cool — certified AC technicians', imageUrl: null, type: TechnicianType.Conditioning },
    { id: 4, title: 'Electrical Safety Check',  subtitle: 'Book now from $25',                    imageUrl: null, type: TechnicianType.Electricity },
  ];

  successResponse(res, { banners, categories, featuredServices, topTechnicians: topProfiles });
};

export const getBanners = async (_req: Request, res: Response): Promise<void> => {
  const banners = [
    { id: 1, title: 'Fix it Fast',            subtitle: 'Professional plumbers near you',        imageUrl: null, type: TechnicianType.Plumbing },
    { id: 2, title: '20% Off Painting',        subtitle: 'Book a painter this week',              imageUrl: null, type: TechnicianType.Painting },
    { id: 3, title: 'AC Service & Repair',     subtitle: 'Certified AC technicians',             imageUrl: null, type: TechnicianType.Conditioning },
    { id: 4, title: 'Electrical Safety Check', subtitle: 'Book now from $25',                    imageUrl: null, type: TechnicianType.Electricity },
    { id: 5, title: 'Carpentry & Woodwork',    subtitle: 'Custom furniture and repairs',          imageUrl: null, type: TechnicianType.Carpentry },
  ];
...
export const getCategories = async (req: Request, res: Response): Promise<void> => {
  ...
  if (type) {
    if (!Object.values(TechnicianType).includes(type as TechnicianType)) {
      errorResponse(res, `Invalid type. Valid values: ${Object.values(TechnicianType).join(', ')}`, 'INVALID_TYPE', 400);
      return;
    }
    filter.technicianType = type;
  }
  const categories = await Category.find(filter).sort({ technicianType: 1, name: 1 });
  successResponse(res, categories);
};

export const getCategoryById = async (req: Request, res: Response): Promise<void> => {
  const category = await Category.findById(req.params.id);
  if (!category) { errorResponse(res, 'Category not found', 'NOT_FOUND', 404); return; }
  successResponse(res, category);
};

export const getCategoryServices = async (req: Request, res: Response): Promise<void> => {
  const { page, limit, skip } = paginate(req.query);
  const filter = { categoryId: req.params.id, isActive: true };
  const [services, total] = await Promise.all([
    Service.find(filter).skip(skip).limit(limit).populate('categoryId', 'name icon technicianType'),
...
export const getServices = async (req: Request, res: Response): Promise<void> => {
  const { category, minPrice, maxPrice, type } = req.query as {
    category?: string; minPrice?: string; maxPrice?: string; type?: string;
  };
  const { page, limit, skip } = paginate(req.query);
  const filter: Record<string, unknown> = { isActive: true };
  ...
  if (type) {
    const cats = await Category.find({ technicianType: type, isActive: true }).distinct('_id');
...
export const getServiceById = async (req: Request, res: Response): Promise<void> => {
  const service = await Service.findById(req.params.id).populate('categoryId', 'name icon technicianType description');
...
export const searchServices = async (req: Request, res: Response): Promise<void> => {
  ...
  if (!q) { errorResponse(res, 'Query param q required', 'MISSING_PARAM', 400); return; }
  const { page, limit, skip } = paginate(req.query);
...
export const getTechnicianTypes = async (_req: Request, res: Response): Promise<void> => {
...
export const searchTechnicians = async (req: Request, res: Response): Promise<void> => {
  const { type, rating, isOnline } = req.query as {
    type?: string; rating?: string; isOnline?: string;
  };
  const { page, limit, skip } = paginate(req.query);
  ...
  if (type) {
    if (!Object.values(TechnicianType).includes(type as TechnicianType)) {
      errorResponse(res, `Invalid type. Valid: ${Object.values(TechnicianType).join(', ')}`, 'INVALID_TYPE', 400);
      return;
    }
    filter.technicianType = type;
  }
...
export const getNearbyTechnicians = async (req: Request, res: Response): Promise<void> => {
  const { lat, lng, radius = '10000', type } = req.query as {
    lat?: string; lng?: string; radius?: string; type?: string;
  };
...
export const getTechnicianById = async (req: Request, res: Response): Promise<void> => {
  const user = await User.findById(req.params.id).select('name avatarUrl email role phone');
...
export const getTechnicianAvailability = async (_req: Request, res: Response): Promise<void> => {
  const slots = [];
  const now = new Date();
  for (let d = 0; d < 7; d++) {
